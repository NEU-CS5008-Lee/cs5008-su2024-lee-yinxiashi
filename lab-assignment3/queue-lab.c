...
void enqueue(int data, int *arr) {
  ...
  // if so we cannot enqueue
  if (isfull()) {
    printf("Queue is full\n");
    return;
  }
  // If not full, increase the pointer of last element;
  // assign the new data to the last pointer
  rear++;
  arr[rear] = data;
...
int dequeue(int *arr) {
  ...
  // for every element, move the position one step ahead
  start = peek(arr);
