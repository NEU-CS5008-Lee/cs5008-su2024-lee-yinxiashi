...
int numberoflistspresent(Graph* graph) {
    int j = 0; // j->number of lists already there in the graph
...
int getIndex(Graph* graph, char* name) {
    int N = graph->numberOfVertices, j = 0;
    for (int i = 0; i < N && strcmp(name, graph->adjLists[i]->name) != 0; i++) {
        j++;
    }
    return j;
}
...
void matrixForm(Graph* graph, int emptyMatrix[50][50], int N) {
    for (int i = 0; i < N; i++) {
        ...
        while (temp != 0) {
            int j = getIndex(graph, temp->name);
            emptyMatrix[i][j] = 1;
            temp = temp->next;
        }
    }
